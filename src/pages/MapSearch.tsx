...
export const MapSearch: React.FC = () => {
  ...
  const filteredProperties = properties.filter((property) => {
    ...
    if (typeFilter) {
      if (typeFilter === 'houses' && property.type !== 'house') typeMatches = false;
      if (typeFilter === 'apartments' && property.type !== 'apartment') typeMatches = false;
      if (typeFilter === 'rooms' && property.type !== 'room') typeMatches = false;
      if (typeFilter === 'master-room' && property.type !== 'master-room') typeMatches = false;
      if (typeFilter === 'plots' && property.type !== 'plot') typeMatches = false;
      if (typeFilter === 'halls' && property.type !== 'hall') typeMatches = false;
    }
      ...
      <div style={{ display: 'flex', flex: 1, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ 
          ...
          <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border-color)' }}>
            <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>{t('map.search')}</h2>
            ...
            {showFilters && (
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  ...
                  <input type="range" min="0" max="1000000" step="50000" value={minPrice} onChange={e => setMinPrice(Number(e.target.value))} style={{ width: '100%', accentColor: 'var(--primary-color)', marginBottom: '6px' }} />
                  <input type="range" min="500000" max="5000000" step="100000" value={maxPrice} onChange={e => setMaxPrice(Number(e.target.value))} style={{ width: '100%', accentColor: 'var(--primary-color)' }} />
                </div>
                <div>
